...
# ... (添加到 db_manager.py 最下面) ...

def update_word_mastery(word_id, success=True):
    """
    更新单词的学习状态
    success: True 表示这次学对了，False 表示学错了
    """
    conn = get_connection()
    c = conn.cursor()
    
    # 简单的艾宾浩斯逻辑：
    # 如果是第一次学会 (status 0 -> 1)，间隔设为 1 天
    # 这里我们简化逻辑：只要学习完成，就标记为 '已学(1)'，并设定明修复习
    
    # 获取当前时间 + 1天
    next_review = (datetime.now() + pd.Timedelta(days=1)).strftime('%Y-%m-%d')
    
    # 更新状态为 1 (Learning/Reviewing)，并设定下次复习时间
    c.execute('''
